...
class EMSampler():
    ...
    def euler_maruyama_step(
        ...
        latent = out.get('latent') if return_latent else None
        ...
        return y_sample, latent

    @torch.no_grad()
    def sample(self, model_fn, flow, noise, batch, capture_at_flow_t=None):
        ...
        # Pre-compute which step index is closest to each target flow-t value.
        # Search only within steps[:sampling_timesteps] to stay within loop bounds.
        capture_steps = {}
        if capture_at_flow_t:
            for target_t in capture_at_flow_t:
                target_tensor = torch.tensor(target_t, device=steps.device)
                idx = int(torch.argmin(torch.abs(steps[:sampling_timesteps] - target_tensor)).item())
                capture_steps[idx] = target_t
        ...
        for i in tqdm(
            ...
            t = steps[i]
            t_next = steps[i + 1]
            need_latent = i in capture_steps

            y_sampled, latent = self.euler_maruyama_step(
                model_fn,
                flow,
                y_sampled,
                t,
            ...
            if need_latent and latent is not None:
                captured[capture_steps[i]] = latent

        return {
            "denoised_coords": y_sampled,
            "captured_latents": captured,
        }
